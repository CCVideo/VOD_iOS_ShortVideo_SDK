...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if(self.toolBarView) {
        ...
        //修改toolBarView frame
        CGFloat toolBarHeight = 105;
        if (@available(iOS 11.0, *)) {
            toolBarHeight += [[UIApplication sharedApplication] delegate].window.safeAreaInsets.bottom > 0 ? 34 : 0;
        }
        
        self.toolBarView.frame = CGRectMake(0.0f, CGRectGetMaxY(self.view.frame) - toolBarHeight , ScreenWidth, toolBarHeight);
        
        [self.toolBarView resetHelpTitle];
...
//        self.toolBarView = [[RTImagePickerToolbarView alloc] initWithFrame:CGRectMake(0.0f, ScreenHeight - toolBarHeight , ScreenWidth, toolBarHeight)];
        self.toolBarView = [[RTImagePickerToolbarView alloc] initWithFrame:CGRectZero];
        ...
        [collectionViewLayout setFooterReferenceSize:CGSizeMake(ScreenWidth, toolBarHeight)];
    }
}
...
- (BOOL)collectionView:(UICollectionView *)collectionView shouldSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self isAutoDeselectEnabled] && !flag) {
        return YES;
    }
