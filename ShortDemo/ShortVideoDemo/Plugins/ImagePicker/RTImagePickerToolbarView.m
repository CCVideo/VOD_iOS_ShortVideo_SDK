...
{
    //界面修改
    previewImage_margin = 10;
    
    previewImage_nextX = 0.0f;
    previewImage_width = 55.0;
    
    CGFloat margin_width = (self.width - 46.0f * 3.0f - 44.0f) /2.0f;
    button_width = 46.0f;
    layoutUpdateAnimateDuration = 0.3f;
        
    self.helpTitleLabel = [[UILabel alloc]initWithFrame:CGRectMake(15, (25 - 12) / 2.0, 137, 12)];
    self.helpTitleLabel.text = @"              ";
//    @"请选择3-30张图片或视频";
    self.helpTitleLabel.font = [UIFont systemFontOfSize:12];
    self.helpTitleLabel.textColor = [UIColor colorWithRed:255/255.0 green:255/255.0 blue:255/255.0 alpha:1.0];
    self.helpTitleLabel.textAlignment = NSTextAlignmentLeft;
    [self.helpTitleLabel sizeToFit];
    [self addSubview:self.helpTitleLabel];
      
    self.photoCountLabel = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMaxX(self.helpTitleLabel.frame) + 5, 0, 22.5, 15)];
    self.photoCountLabel.center = CGPointMake(self.photoCountLabel.center.x, self.helpTitleLabel.center.y);
    self.photoCountLabel.text = @"0";
    self.photoCountLabel.font = [UIFont systemFontOfSize:12];
    self.photoCountLabel.textColor = [UIColor colorWithRed:255/255.0 green:255/255.0 blue:255/255.0 alpha:1.0];
    self.photoCountLabel.textAlignment = NSTextAlignmentCenter;
    self.photoCountLabel.layer.masksToBounds = YES;
    self.photoCountLabel.layer.cornerRadius = 7.5;
    self.photoCountLabel.backgroundColor = [UIColor colorWithRed:255/255.0 green:149/255.0 blue:32/255.0 alpha:1.0];
    [self addSubview:self.photoCountLabel];
    
    UILabel * tsLabel2 = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMaxX(self.photoCountLabel.frame) + 10 - 15, self.helpTitleLabel.frame.origin.y, ScreenWidth - CGRectGetMaxX(self.photoCountLabel.frame) - 10, self.helpTitleLabel.frame.size.height)];
    tsLabel2.text = @"长按调整顺序";
    tsLabel2.font = [UIFont systemFontOfSize:12];
    tsLabel2.textColor = [UIColor colorWithRed:255/255.0 green:255/255.0 blue:255/255.0 alpha:0.5];
    tsLabel2.textAlignment = NSTextAlignmentRight;
    [self addSubview:tsLabel2];
    
//    self.imagePickerToolbarBackgroundView = [[UIView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, ScreenWidth, self.height)];
    self.imagePickerToolbarBackgroundView = [[UIView alloc] initWithFrame:CGRectMake(0.0f, 25 + (80 - previewImage_width) / 2.0, ScreenWidth, previewImage_width)];
    //    _imagePickerToolbarBackgroundView.backgroundColor = self.backgroundColor;
...
//    self.previewScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0.0f, self.height/2.0f, ScreenWidth, self.height/2.0f)];
    self.previewScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0.0f, 0, ScreenWidth, previewImage_width)];

    _previewScrollView.showsVerticalScrollIndicator = NO;
//    _previewScrollView.backgroundColor = _imagePickerToolbarBackgroundView.backgroundColor;
...
//    self.photoBrowserToolbarBackgroundView = [[UIView alloc]initWithFrame:CGRectMake(0.0f, self.height, ScreenWidth, self.height)];
...
- (void)resetHelpTitle
{
    self.helpTitleLabel.text = self.viewController.imagePickerController.mediaType == RTImagePickerMediaTypeVideo ? @"请选择视频1个视频" : @"请选择3-30张图片";
    [self.helpTitleLabel sizeToFit];
    self.photoCountLabel.frame = CGRectMake(CGRectGetMaxX(self.helpTitleLabel.frame) + 5, self.photoCountLabel.frame.origin.y, 22.5, 15);
}
...
-(void)deleteAssetButtonAction:(UIButton *)button
{
    UIImageView * imageView = (UIImageView *)button.superview;
    
    NSInteger currentImageIndex = [self.previewImageViewArray indexOfObject:imageView];
    ...
    [UIView animateWithDuration:0.2f animations:^{
        ...
        [self deleteAssetAtIndex:currentImageIndex];
...
- (void)addAsset:(PHAsset *)asset
{
    ...
    self.viewController.imagePickerController.chooseAssets = [self.selectedAssets copy];
...
//    [self updateLayoutWhenUpdatingAsset];
    ...
    UIImageView *previewImageView = [[UIImageView alloc]initWithFrame:CGRectMake(previewImage_nextX, 0, previewImage_width, previewImage_width )];
    ...
    //增加删除按钮
    UIButton * deleteAssetButton = [UIButton buttonWithType:UIButtonTypeCustom];
    deleteAssetButton.frame = CGRectMake(previewImage_width - 12, 0, 12, 12);
    [deleteAssetButton setBackgroundImage:[UIImage imageNamed:@"icon_photo_delete.png"] forState:UIControlStateNormal];
    [deleteAssetButton addTarget:self action:@selector(deleteAssetButtonAction:) forControlEvents:UIControlEventTouchUpInside];
    [previewImageView addSubview:deleteAssetButton];
...
{
    self.photoCountLabel.text = [NSString stringWithFormat:@"%ld",self.selectedAssets.count];
}
        ...
        case UIGestureRecognizerStateEnded:
        {
            ...
            if(translation.y <  - previewImage_width) {
                ...
                if(index_end == currentPanImageViewIndex) {
                    ...
                    PHAsset * asset = [self.selectedAssets objectAtIndex:currentPanImageViewIndex];
                    [self.selectedAssets removeObject:asset];
                    if (index_end >= _selectedAssets.count) {
                        [_selectedAssets addObject:asset];
                    }else{
                        [_selectedAssets insertObject:asset atIndex:index_end];
                    }
