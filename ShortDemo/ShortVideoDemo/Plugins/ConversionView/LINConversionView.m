...
@property (strong, nonatomic) UIImageView *rotateCtrl;//控制关闭，左上角
@property (strong, nonatomic) UIImageView *resizeCtrl;//控制时长，右上角
@property (strong, nonatomic) UIImageView *textCtrl;//控制文字，左下角
...
//                                  control_radius * 2);
        ...
        [_rotateCtrl addGestureRecognizer:deleteTap];
...
//        _resizeCtrl.autoresizingMask = UIViewAutoresizingFlexibleRightMargin | UIViewAutoresizingFlexibleTopMargin;
        _resizeCtrl.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleBottomMargin;
        ...
        [_resizeCtrl addGestureRecognizer:timeTap];
...
- (UIImageView *)textCtrl
{
    if (!_textCtrl) {
        CGRect frame = CGRectMake(0 - control_radius,
                                  ...
                                  control_radius * 2,
                                  control_radius * 2);
        _textCtrl = [[UIImageView alloc]initWithFrame:frame];
        _textCtrl.userInteractionEnabled = YES;
        _textCtrl.autoresizingMask = UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleRightMargin;
        [self addSubview:_textCtrl];

        UITapGestureRecognizer * textTap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(textTapAction)];
        [_textCtrl addGestureRecognizer:textTap];
    }
    return _textCtrl;
}
...
- (void)pan:(UIPanGestureRecognizer *)gesture {
    ...
    if (gesture.state == UIGestureRecognizerStateEnded) {
        if ([self.delegate respondsToSelector:@selector(conversionViewEndPanGesture:)]) {
            [self.delegate conversionViewEndPanGesture:self];
        }
    }
}
...
{
    if ([self.delegate respondsToSelector:@selector(conversionViewDidDeleteAction:)]) {
        [self.delegate conversionViewDidDeleteAction:self];
    }
}

-(void)timeTapAction
{
    if ([self.delegate respondsToSelector:@selector(conversionViewDidTimeAction:)]) {
        [self.delegate conversionViewDidTimeAction:self];
    }
}

-(void)textTapAction
{
    if ([self.delegate respondsToSelector:@selector(conversionViewDidTextAction:)]) {
        [self.delegate conversionViewDidTextAction:self];
    }
}
...
- (void)fitCtrlScaleX:(CGFloat)scaleX scaleY:(CGFloat)scaleY {
    ...
    _textCtrl.transform = CGAffineTransformScale(_textCtrl.transform, 1/scaleX, 1/scaleY);
    _oCtrlPointView.transform = CGAffineTransformScale(_oCtrlPointView.transform, 1/scaleX, 1/scaleY);
    
    self.contentView.transform = CGAffineTransformScale(self.contentView.transform, scaleX, scaleY);
}
...
-(void)editTapAction
{
    ...
    self.transformCtrl.hidden = !self.isEdit;
    self.rotateCtrl.hidden = !self.isEdit;
    self.resizeCtrl.hidden = !self.isEdit;
    self.textCtrl.hidden = !self.isEdit;
    
    if ([self.delegate respondsToSelector:@selector(conversionViewEditValueChange:)]) {
        [self.delegate conversionViewEditValueChange:self];
    }
}
...
-(void)setIsEdit:(BOOL)isEdit
{
    ...
    self.transformCtrl.hidden = !_isEdit;
    self.rotateCtrl.hidden = !_isEdit;
    self.resizeCtrl.hidden = !_isEdit;
    self.textCtrl.hidden = !_isEdit;
}
...
- (void)setTransformCtrlImage:(UIImage *)image resizeCtrlImage:(UIImage *)resizeImage rotateCtrlImage:(UIImage *)rotateImage
{
    self.transformCtrl.backgroundColor = [UIColor clearColor];
    self.transformCtrl.image = image;
    self.resizeCtrl.backgroundColor = [UIColor clearColor];
    ...
    self.resizeCtrl.image = resizeImage;
    self.rotateCtrl.image = rotateImage;
    self.textCtrl.hidden = YES;
}

- (void)setTransformCtrlImage:(UIImage *)image resizeCtrlImage:(UIImage *)resizeImage rotateCtrlImage:(UIImage *)rotateImage TextCtrlImage:(UIImage *)textImage
{
    self.transformCtrl.backgroundColor = [UIColor clearColor];
    self.transformCtrl.image = image;
    self.resizeCtrl.backgroundColor = [UIColor clearColor];
    ...
    self.resizeCtrl.image = resizeImage;
    self.rotateCtrl.image = rotateImage;
    self.textCtrl.hidden = NO;
    self.textCtrl.image = textImage;
    self.textCtrl.backgroundColor = [UIColor clearColor];
}
