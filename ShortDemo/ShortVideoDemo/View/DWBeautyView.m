...
-(void)sliderValueChange:(UISlider *)slider
{
    if (slider.tag == 100) {
        ...
        [self.delegate DWBeautyViewWhiteValueChange:(NSInteger)slider.value];
    }
    
    if (slider.tag == 101) {
        self.microderLabel.text = [NSString stringWithFormat:@"%.0f",slider.value];
        [self.delegate DWBeautyViewMicroderValueChange:(NSInteger)slider.value];
    }
}
...
-(void)dismiss
{
    ...
    [self.delegate DWBeautyViewDismiss];
}
